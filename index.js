...
(async function main() {
  try {
    ...
    await page.goto("https://covidwb.com/oxygen");
    const title=await page.title();
    await console.log(`Scraping ${title} for data.....`);
   
    await page.waitForSelector(".mobile-search-results");

    await page.click(
      ".rounded-pill.m-1.filter-option.btn.btn-outline-secondary.btn-sm",
    );
    await page.click(
      ".rounded-pill.m-1.filter-option.btn.btn-outline-secondary.btn-sm"
    );
    ...
    const grabNumbers = await page.evaluate(() => {
      const phonelist = document.querySelectorAll(
        ".mb-1.mr-1.row .col .text-primary.mt-1"
      );
      const phoneNumbers = [];
    ...
    const grabAddress = await page.evaluate(() => {
      const addresslist = document.querySelectorAll(
        ".col-auto.col .m-1.p-1.info-chip.text-wrap.badge.badge-light"
      );
      const addresses = [];
      addresslist.forEach((element) => {
        if (
          element.innerHTML != "Refill" &&
          element.innerHTML != "Cylinder" &&
          element.innerHTML != "Can" &&
          element.innerHTML != "Concentrator"
        ) {
    ...
    const grabTime = await page.evaluate(() => {
      const timelist = document.querySelectorAll(
        ".badge-pill.last-updated.p-2.small.badge.badge-light"
      );
      const times = [];
    ...
    console.log(rows);
    var csv = 'Name,Phone,Address,Time of Verification\n';  
    ...
    await fs.writeFile("./oxygen-wb.csv", csv, function(err) {
        if(err) {
